...
function _getSoapBaseUrl(client, origin, stack, soapOrigin, globalReqOptions, cacheLocation, callback) {
    ...
    if (soapOrigin) { // if base url fetched, run callback
        ...
        const cache = new CacheService(client.AuthClient, cacheLocation);
        cache.get().then(({url}) => {
            if (url) {
...
var ET_Client = function(clientId, clientSecret, stack, {origin, authOrigin, soapOrigin, globalReqOptions = {}} = {}, cacheLocation = null) {
	...
	_stack=stack;
	this.FuelAuthClient = this.initAuth(clientId, clientSecret, authOrigin, globalReqOptions);
	this.RestClient = this.initRest(this.FuelAuthClient, origin, globalReqOptions);
	this.SoapClient = this.initSoap(this.FuelAuthClient, origin, stack, soapOrigin, globalReqOptions, cacheLocation);

};

ET_Client.prototype.initAuth = function(clientId, clientSecret, authOrigin, globalReqOptions) {
...
ET_Client.prototype.initSoap = function(auth, origin, stack, soapOrigin, globalReqOptions, cacheLocation) {
    ...
    ['create', 'retrieve', 'update', 'delete'].forEach(method => { // for each method
        client[method] = function () {
            ...
            _getSoapBaseUrl(client, origin, stack, soapOrigin, globalReqOptions, cacheLocation, () => { // get the custom base url
